...
import { laporanHarianSchema, laporanKesehatanSchema } from "../schema/aktivitasSchema.js";

export const getLaporanHarianById = async (req, res, next) => { // Tambahkan next
    try {
        ...
        if (!laporan) {
            return next(new AppError(`Laporan  dengan ID ${id_laporan} tidak ditemukan.`, 404, 'LAPORAN_NOT_FOUND'));
        }
...
export const getLaporanKesehatanByNik = async (req, res, next) => {
    try {
        ...
        if (!nik) {
            return next(new AppError('Parameter NIK dibutuhkan di URL.', 400, 'NIK_PARAM_REQUIRED'));
        }
        ...
        if (!laporan) {
            return next(new AppError(`Laporan kesehatan tidak ditemukan.`, 404, 'LAPORAN_KESEHATAN_NOT_FOUND'));
        }
...
export const getLaporanKesehatanById = async (req, res, next) => {
    try {
        ...
        if (!id_laporan) {
            return next(new AppError('Parameter id_laporan dibutuhkan di URL.', 400, 'ID_LAPORAN_PARAM_REQUIRED'));
        }
        ...
        if (!laporan) {
            return next(new AppError(`Laporan kesehatan dengan ID ${id_laporan} tidak ditemukan.`, 404, 'LAPORAN_KESEHATAN_NOT_FOUND'));
        }
...
export const getLaporanKesehatanByTanggal = async (req, res, next) => {
    try {
        ...
        if (!nik || !tanggal) {
            return next(new AppError('Field NIK dan Tanggal dibutuhkan dalam body request.', 400, 'MISSING_NIK_TANGGAL_BODY'));
        }
...
export const createLaporanHarian = async (req, res, next) => { // Tambahkan next
    try {
        const validationResult = laporanHarianSchema.safeParse(req.body);

        if (!validationResult.success) {
            const formattedErrors = validationResult.error.flatten().fieldErrors;
            return next(new AppError('Data input tidak valid.', 422, 'VALIDATION_ERROR', formattedErrors));
        }
        ...
        if (error.name === 'SequelizeUniqueConstraintError') {
            return next(new AppError('Gagal menambahkan data: Kombinasi data sudah ada (cek unique constraints).', 409, 'DUPLICATE_ENTRY', error.errors));
        }
...
export const createLaporanKesehatan = async (req, res, next) => {
    try {
        ...
        if (error.name === 'SequelizeUniqueConstraintError') {
            return next(new AppError('Gagal menambahkan data: Laporan untuk ID/NIK/Tanggal tersebut mungkin sudah ada.', 409, 'DUPLICATE_LAPKES', error.errors));
        }
        if (error.name === 'SequelizeValidationError') {
            return next(new AppError('Data tidak valid (Sequelize).', 400, 'SEQUELIZE_VALIDATION_ERROR', error.errors));
        }
...
export const deleteLaporanHarianByNoUrut = async (req, res, next) => {
    try {
        ...
        if (!id_laporan || !no_urut) {
            return next(new AppError('Field id_laporan dan no_urut dibutuhkan dalam body request.', 400, 'MISSING_DELETE_BODY_PARAMS'));
        }
        ...
        if (deleteLaporan === 0) {
            return next(new AppError(`Detail laporan dengan ID ${id_laporan} dan No Urut ${no_urut} tidak ditemukan.`, 404, 'LAPORAN_DETAIL_NOT_FOUND'));
        }

        res.success(null, `Detail laporan dengan No Urut ${no_urut} untuk ID Laporan ${id_laporan} berhasil dihapus.`);
...
export const deleteLaporanHarianById = async (req, res, next) => {
    try {
        ...
        // --- Validasi Input ---
        if (!id_laporan) {
            return next(new AppError('Field id_laporan dibutuhkan dalam body request.', 400, 'ID_LAPORAN_BODY_REQUIRED'));
        }
        ...
        res.success(null, `Berhasil menghapus ${deleteLaporan} detail laporan untuk ID Laporan ${id_laporan}.`);
...
export const deleteLaporanKesehatanById = async (req, res, next) => {
    try {
        ...
        if (deleteLaporan === 0) {
            return next(new AppError(`Laporan kesehatan dengan ID ${id_laporan} tidak ditemukan.`, 404, 'LAPORAN_KES_NOT_FOUND'));
        }

        res.success(null, `Laporan kesehatan dengan ID ${id_laporan} berhasil dihapus.`);
