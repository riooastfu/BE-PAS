...
export const verifyToken = async (req, res, next) => {
    try {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
            return next(new AppError('Tidak ada token yang diberikan', 401, 'NO_TOKEN_PROVIDED'));
        }
...
export const verifyRefreshToken = async (req, res, next) => {
    try {
        ...
        if (!refreshToken) {
            return next(new AppError('Refresh token tidak diberikan', 400, 'NO_REFRESH_TOKEN'));
        }
        ...
        if (!tokenDoc) {
            return next(new AppError('Refresh token tidak valid', 401, 'INVALID_REFRESH_TOKEN'));
        }
        ...
        if (new Date() > new Date(tokenDoc.expires_at)) {
            ...
            return next(new AppError('Refresh token kedaluwarsa. Silakan login kembali.', 401, 'REFRESH_TOKEN_EXPIRED'));
        }
        ...
        if (error.name === 'TokenExpiredError') {
            return next(new AppError('Refresh token kedaluwarsa', 401, 'REFRESH_TOKEN_EXPIRED'));
        } else if (error.name === 'JsonWebTokenError') {
            return next(new AppError('Refresh token tidak valid', 401, 'INVALID_REFRESH_TOKEN'));
        }
        next(error);
...
export const checkRole = (allowedRoles) => {
    return (req, res, next) => {
        ...
        if (!req.user || !req.user.id_role) {
            ...
            return next(new AppError('Informasi peran pengguna tidak ditemukan dalam token.', 401, 'MISSING_ROLE_INFO'));
        }
        ...
        if (allowedRoles.includes(userRole)) {
            ...
            return next(new AppError('Anda tidak memiliki izin untuk mengakses sumber daya ini.', 403, 'FORBIDDEN_ACCESS'));
        }
    };
};